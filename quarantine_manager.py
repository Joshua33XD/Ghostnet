    ...
    def _evaluate_node(self, node_id: str) -> None:
        ...
        for threat_name in active_threats:
            if threat_name not in sent_protect:
                ...
                self._store.set_protection(
                    node_id, threat_name, protect_cmd, f"[{label}] {reason}",
        ...
        for threat_name in list(cleared):
            ...
            self._store.update_protection_streak(node_id, threat_name, streak)
            ...
            if streak >= config.RECOVERY_WINDOW:
                ...
                heal_streaks[threat_name] = 0
                self._store.clear_protection(node_id, threat_name)
                logger.info(
            ...
            if node.status == NodeStatus.QUARANTINED:
                ...
                if score < config.RECOVERY_THRESHOLD:
                    ...
                    if streak >= config.RECOVERY_WINDOW:
                        ...
                        self._store.clear_protection(node_id, "overall_anomaly")
        ...
        # -- STEP 4: Escalate to QUARANTINE if overall score still high ---------
            ...
            if node.anomaly_score >= config.ANOMALY_THRESHOLD:
                ...
                self._store.set_protection(
                    node_id, "overall_anomaly", RESPONSE_QUARANTINE,
                    f"Fused anomaly score {node.anomaly_score:.2f} exceeded threshold "
                    f"({config.ANOMALY_THRESHOLD}).",
                    config.RECOVERY_WINDOW,
                )
