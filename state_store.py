...
class NodeState:
    ...
    # ── Per-threat active protection state (what action is live + heal progress) ─
    # { threat_name: {"action": str, "reason": str, "heal_streak": int, "recovery_window": int} }
    active_protections: Dict[str, dict] = field(default_factory=dict)
...
class StateStore:
    ...
    def set_protection(self, node_id: str, threat_name: str, action: str,
                       reason: str, recovery_window: int) -> None:
        with self._lock:
            node = self.get_or_create(node_id)
            node.active_protections[threat_name] = {
                "action": action, "reason": reason,
                "heal_streak": 0, "recovery_window": recovery_window,
            }

    def update_protection_streak(self, node_id: str, threat_name: str, streak: int) -> None:
        with self._lock:
            node = self.get_or_create(node_id)
            entry = node.active_protections.get(threat_name)
            if entry:
                entry["heal_streak"] = streak

    def clear_protection(self, node_id: str, threat_name: str) -> None:
        with self._lock:
            node = self.get_or_create(node_id)
            node.active_protections.pop(threat_name, None)
